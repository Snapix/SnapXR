  ...
  return (
    <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '8px' }}>
      {widgetList.map(w => {
        ...
        return (
          <button key={w.type} className="btn-secondary"
            onClick={() => onAddWidget(w.type)}
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', minWidth: '100px', flexShrink: 0, position: 'relative' }}
          >
            <span style={{ fontSize: '24px' }}>{w.icon}</span>
            <span style={{ fontSize: '10px' }}>{w.label}</span>
            {count > 0 && (
              <div style={{ background: 'var(--accent)', color: '#000', borderRadius: '50%', width: '16px', height: '16px', fontSize: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'absolute', top: '4px', right: '4px' }}>
                {count}
              </div>
            )}
          </button>
